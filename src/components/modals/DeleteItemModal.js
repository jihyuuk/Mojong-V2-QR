...
function DeleteItemModal({ item, close }) {
    ...
    //상품 삭제
    const deleteItem = () => {
        axiosWithToken.delete(`/item/${item.id}`)
            .then((resopne) => {
                close();
                fetchMenu();
                showTost("상품 삭제 성공");
        ...
        <Modal show={true} style={{ paddingTop: '10rem' }} >
            <ModalHeader className='border-bottom border-success-subtle border-2'>
                <ModalTitle className="d-flex align-items-center">
                    ...
                    <div className='ms-2 text-danger fw-semibold'>상품 삭제</div>
                </ModalTitle>
            </ModalHeader>

            <ModalBody className='text-center py-5 fs-4'>
                <div>
                    <div className='text-success fw-semibold pb-2'>{item.name}</div>
            ...
            <div className='d-flex gap-1 p-2 border-top'>
                ...
                <Button variant='danger' className='w-100' onClick={deleteItem}>삭제하기</Button>
            </div>
        </Modal>
    )
}
