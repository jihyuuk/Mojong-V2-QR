...
function AddCategoryModal({ close }) {
    ...
    const addCategory = () => {
        ...
        axiosWithToken.post("/category/new", inputValue.trim(), { headers: { 'Content-Type': 'text/plain' } })
            .then((resopne) => {
                close();
                fetchMenu();
                showTost("카테고리 추가 성공");
        ...
        <Modal show={true} animation={false} >
            <ModalHeader className='border-bottom border-success-subtle border-2'>
                <ModalTitle className="d-flex align-items-center">
                    ...
                    <div className='ms-2 fw-medium'>카테고리 추가</div>
                </ModalTitle>
            </ModalHeader>

            <ModalBody className='py-5 fs-4'>
                <FormLabel className='fw-medium pb-3 fs-5'>카테고리명</FormLabel>
                <FormControl className='mb-3 fs-5' autoFocus value={inputValue} onChange={(e) => changeInput(e.target.value)} isInvalid={invalid}></FormControl>
            ...
            <div className='d-flex gap-1 p-2 border-top'>
                ...
                <Button variant='success' className='w-100' onClick={addCategory}>추가하기</Button>
            </div>
        </Modal>
    )
}
